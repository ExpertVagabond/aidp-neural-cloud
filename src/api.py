...
def validate_message_content(content: str) -> str:
    """Validate message content length and sanitize control characters."""
...
class ChatCompletionRequest(BaseModel):
    ...
    @classmethod
    def validate_stop(cls, v: Optional[list]) -> Optional[list]:
        if v and len(v) > MAX_STOP_SEQUENCES:
            raise ValueError(f"Too many stop sequences (max {MAX_STOP_SEQUENCES})")
        return v
